...
function initMap(){
  ...
  if(!map){
    ...
    L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png',{maxZoom:20,subdomains:'abcd',attribution:'© OpenStreetMap © CARTO'}).addTo(map);
...
function drawRoadRoute(){if(routeLine)routeLine.remove();if(routeHalo)routeHalo.remove();const coords=currentRoute.geometry.coordinates.map(c=>[c[1],c[0]]);routeHalo=L.polyline(coords,{weight:13,opacity:.96,color:'#ffffff',lineCap:'round',lineJoin:'round',interactive:false}).addTo(map);routeLine=L.polyline(coords,{weight:7,opacity:1,color:'#168bd2',lineCap:'round',lineJoin:'round',interactive:false}).addTo(map);routeHalo.bringToFront();routeLine.bringToFront();setTimeout(()=>map.invalidateSize({pan:false}),40);if(lastUserPos&&followUser){map.setView([lastUserPos.lat,lastUserPos.lon],17,{animate:true})}else{map.fitBounds(routeLine.getBounds(),{paddingTopLeft:[45,210],paddingBottomRight:[45,255],maxZoom:17})}}
