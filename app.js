...
function defaults(){return{date:todayIso(),settings:{startAddress:'Roquefort-les-Pins 06330, France',storageAddress:'Roquefort-les-Pins 06330, France'},stops:[],history:[]}}
...
function sync(){ $('tourDate').value=state.date;$('startAddress').value=state.settings.startAddress||'';$('storageAddress').value=state.settings.storageAddress||'';renderStops() }
$('tourDate').onchange=e=>{state.date=e.target.value;save();renderStops()};
$('saveSettingsBtn').onclick=()=>{state.settings.startAddress=$('startAddress').value.trim();state.settings.storageAddress=$('storageAddress').value.trim();save();alert('Paramètres enregistrés.');};
...
function canApply(stock,s){const z=q(s);if(s.type==='pickup')return true;return stock.tables>=z.tables&&stock.chairs>=z.chairs&&stock.tents>=z.tents}
...
async function optimize(){if(!state.stops.length){alert('Ajoute au moins une intervention.');return}showView('loading');$('loadingMessage').textContent='Géolocalisation des adresses…';try{const start=await geocode(state.settings.startAddress||state.settings.storageAddress);for(const stop of state.stops){if(!stop.geo)stop.geo=await geocode(stop.address)}save();$('loadingMessage').textContent='Calcul des temps de route réels…';let matrix=null;try{matrix=await roadMatrix([start,...state.stops.map(stop=>stop.geo)])}catch(e){console.warn('Matrice routière indisponible, repli distance directe',e)}$('loadingMessage').textContent='Optimisation du chargement et de l’ordre de passage…';const remaining=state.stops.map((_,i)=>i),seq=[];let posIndex=0,pos=start;let initial={tables:0,chairs:0,tents:0};let stock={...initial};const travelCost=i=>matrix?.durations?(matrix.durations[posIndex][i+1]??1e12):dist(pos,state.stops[i].geo);while(remaining.length){let feasible=remaining.filter(i=>canApply(stock,state.stops[i]));if(!feasible.length){const deliveries=remaining.filter(i=>state.stops[i].type==='delivery').sort((a,b)=>travelCost(a)-travelCost(b));if(!deliveries.length)throw new Error('Impossible de déterminer la prochaine étape.');const target=deliveries[0],z=q(state.stops[target]);for(const k of ['tables','chairs','tents']){const missing=Math.max(0,z[k]-stock[k]);if(missing){initial[k]+=missing;stock[k]+=missing}}feasible=remaining.filter(i=>canApply(stock,state.stops[i]))}feasible.sort((a,b)=>travelCost(a)-travelCost(b));const pick=feasible[0];seq.push(pick);stock=apply(stock,state.stops[pick]);pos=state.stops[pick].geo;posIndex=pick+1;remaining.splice(remaining.indexOf(pick),1)}const stocks=[];stock={...initial};seq.forEach(i=>{stock=apply(stock,state.stops[i]);stocks.push({...stock})});plan={sequence:seq,initialLoad:initial,stocks,endStock:stock};setTimeout(showLoadPlan,350)}catch(err){console.error(err);alert(err.message);showView('home')}}
