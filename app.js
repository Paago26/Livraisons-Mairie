...
let state=load(),plan=null,currentIndex=0,map=null,userMarker=null,destMarker=null,routeLine=null;
let currentRoute=null,lastUserPos=null,lastRouteAt=0,routeRequestSeq=0,watchId=null,followUser=true;
function save(){localStorage.setItem(STORAGE_KEY,JSON.stringify(state))}
function showView(name){document.querySelectorAll('.view').forEach(v=>v.classList.remove('active-view'));$(name+'View').classList.add('active-view');closeDrawer();if(name==='history')renderHistory();}
...
$('closeEntryBtn').onclick=()=>$('entryDialog').close();
$('closePrevBtn').onclick=()=>$('previousPickupsDialog').close();
function editStop(id){openEntry(state.stops.find(x=>x.id===id)?.type,state.stops.find(x=>x.id===id))}
$('entryForm').onsubmit=e=>{if(e.submitter?.value==='cancel')return; e.preventDefault();const s={id:$('entryId').value||uid(),type:$('entryType').value,lastName:$('lastName').value.trim(),firstName:$('firstName').value.trim(),address:$('address').value.trim(),phone:$('phone').value.trim(),tables:n($('tables').value),chairs:n($('chairs').value),tents:n($('tents').value),notes:$('notes').value.trim(),returnDate:$('entryType').value==='delivery'?$('returnDate').value:null,status:'planned'};if(!s.address)return;const idx=state.stops.findIndex(x=>x.id===s.id);if(idx>=0)state.stops[idx]=s;else state.stops.push(s);save();renderStops();$('entryDialog').close()};
...
$('startNavigationBtn').onclick=()=>{currentIndex=0;showView('navigation');setTimeout(initMap,80);renderCurrentStop()};
function initMap(){
  const el=$('map');
  if(!el)return;
  if(!map){
...
function onGps(p){
  const ll={lat:p.coords.latitude,lon:p.coords.longitude,accuracy:p.coords.accuracy};lastUserPos=ll;
  const latlng=[ll.lat,ll.lon];
  if(!userMarker){userMarker=L.circleMarker(latlng,{radius:9,weight:4,fillOpacity:1}).addTo(map)}else userMarker.setLatLng(latlng);
...
function drawRoadRoute(){if(routeLine)routeLine.remove();const coords=currentRoute.geometry.coordinates.map(c=>[c[1],c[0]]);routeLine=L.polyline(coords,{weight:8,opacity:.95,lineCap:'round',lineJoin:'round'}).addTo(map);routeLine.bringToFront();setTimeout(()=>map.invalidateSize({pan:false}),30);if(!lastUserPos||!followUser)map.fitBounds(routeLine.getBounds(),{paddingTopLeft:[45,190],paddingBottomRight:[45,260],maxZoom:17});}
function drawFallbackLine(){if(!map||!destMarker)return;if(routeLine)routeLine.remove();const from=userMarker?.getLatLng();if(!from)return;routeLine=L.polyline([from,destMarker.getLatLng()],{weight:6,dashArray:'10 10',opacity:.7}).addTo(map)}
function renderRouteInfo(){if(!currentRoute)return;$('routeDistance').textContent=formatDistance(currentRoute.distance);$('routeDuration').textContent=formatDuration(currentRoute.duration);$('routeEta').textContent=etaFrom(currentRoute.duration);const steps=currentRoute.legs?.[0]?.steps||[];let step=steps.find(x=>x.distance>18&&x.maneuver?.type!=='depart')||steps[0];if(step){$('maneuverIcon').textContent=maneuverIcon(step.maneuver?.type,step.maneuver?.modifier);$('maneuverText').textContent=instructionFr(step);$('maneuverDistance').textContent=step.maneuver?.type==='arrive'?'':`dans ${formatDistance(step.distance)}`}}
function renderCurrentStop(){const s=currentStop();if(!s){finishTour();return}$('currentDestination').textContent=s.address;$('currentType').textContent=(s.type==='delivery'?'Livraison':'Récupération')+' — '+([s.firstName,s.lastName].filter(Boolean).join(' ')||'Sans nom');$('progressText').textContent=`Étape ${currentIndex+1} / ${plan.sequence.length}`;$('truckStockText').textContent='Après arrêt : '+qText(plan.stocks[currentIndex]);$('callBtn').href=s.phone?'tel:'+s.phone:'#';$('callBtn').classList.toggle('disabled',!s.phone);$('externalNavBtn').href=`https://maps.apple.com/?daddr=${encodeURIComponent(s.address)}&dirflg=d`;if(destMarker)destMarker.remove();destMarker=L.marker([s.geo.lat,s.geo.lon]).addTo(map);followUser=true;currentRoute=null;setTimeout(()=>{if(map){map.invalidateSize({pan:false});if(!lastUserPos)map.setView([s.geo.lat,s.geo.lon],15)}requestRoute(true)},180)}
